...
mod tests {
        ...
        pub fn start<F: Future<Output = ()>>(
            ...
            executor.start(|context| async move {
                ...
                let participants = Committee::try_from(
                    peer_keys.iter().cloned().zip(weights).collect::<Vec<_>>(),
                )
    ...
    #[test_traced]
    fn test_notarized_commitment_reconstructs_from_dominant_voter() {
        let fixture: Fixture<C> = Fixture {
        ...
        fixture.start(
            |config, context, _, mut peers, _, coding_config| async move {
                ...
                let reconstructed = peers[receiver_idx]
                    ...
                    .get(commitment)
                    .await
                    .expect("notarized block should reconstruct from dominant participant shard");
                assert_eq!(reconstructed.commitment(), commitment);
            },
        );
    }
