...
mod tests {
    ...
    use commonware_utils::{
        NZU16, NZU64, NZUsize, NonZeroDuration, acknowledgement::Exact, non_empty, ordered::Set,
        sync::Mutex,
    };
    ...
    fn active_horizon_stops_engines_and_keeps_epoch_schemes_isolated() {
        deterministic::Runner::timed(Duration::from_secs(20)).start(|mut context| async move {
            ...
            let participant_set = |indices: &[usize]| {
                Set::from_iter_dedup(
                    indices
                        .iter()
                        .map(|index| fixture.participants[*index].clone()),
                )
            };
            let epoch_schemes = [
                ...
                TestScheme::signer(
                    NAMESPACE,
                    participant_set(&[0, 1, 2, 3, 4]),
                    fixture.private_keys[0].clone(),
                )
                .unwrap(),
                TestScheme::signer(
                    NAMESPACE,
                    participant_set(&[0, 3, 4]),
                    fixture.private_keys[0].clone(),
                )
                .unwrap(),
                TestScheme::signer(
