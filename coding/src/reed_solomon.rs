...
mod tests {
    ...
    #[test]
    fn test_validate_config_supported_boundary() {
        let config = Config {
            minimum_shards: NZU16!(32_768),
            extra_shards: NZU16!(32_767),
        };

        assert!(RS::validate_config(&config).is_ok());
    }

    #[test]
    fn test_validate_config_unsupported_shard_count() {
        let config = Config {
    ...
    #[test]
    fn test_validate_config_total_overflow() {
        let config = Config {
        ...
        assert!(matches!(
            RS::validate_config(&config),
            Err(Error::TooManyTotalShards(65_536))
        ));
    }
